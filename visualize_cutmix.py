import os
import torch
import numpy as np
from PIL import Image
import torchvision.transforms as T
import matplotlib.pyplot as plt

def rand_bbox(size, lam):
    """
    Generates a random bounding box for CutMix.
    """
    W = size[2]
    H = size[1]
    cut_rat = np.sqrt(1. - lam)
    cut_w = int(W * cut_rat)
    cut_h = int(H * cut_rat)

    # Uniformly sample the center of the box
    cx = np.random.randint(W)
    cy = np.random.randint(H)

    # Calculate box coordinates, clamping to image boundaries
    bbx1 = np.clip(cx - cut_w // 2, 0, W)
    bby1 = np.clip(cy - cut_h // 2, 0, H)
    bbx2 = np.clip(cx + cut_w // 2, 0, W)
    bby2 = np.clip(cy + cut_h // 2, 0, H)

    return bbx1, bby1, bbx2, bby2

def visualize_cutmix(image_dir: str, alpha: float = 1.0):
    """
    Finds two images, applies CutMix, and saves a visualization.
    """
    # --- 1. Validate directory and find the two image files ---
    if not os.path.isdir(image_dir):
        print(f"Error: Directory not found at '{image_dir}'")
        return

    try:
        image_names = [f for f in os.listdir(image_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))]
        if len(image_names) != 2:
            print(f"Error: Expected exactly 2 images in '{image_dir}', but found {len(image_names)}.")
            return
        
        img_path1 = os.path.join(image_dir, image_names[0])
        img_path2 = os.path.join(image_dir, image_names[1])

    except Exception as e:
        print(f"Error while accessing directory or finding images: {e}")
        return

    # --- 2. Define transformation and load images ---
    transform = T.Compose([
        T.Resize((256, 256)),
        T.ToTensor(),
    ])

    try:
        with Image.open(img_path1).convert('RGB') as img1, Image.open(img_path2).convert('RGB') as img2:
            tensor1 = transform(img1)
            tensor2 = transform(img2)
    except Exception as e:
        print(f"Error opening or transforming images: {e}")
        return

    # --- 3. Apply CutMix ---
    lam = np.random.beta(alpha, alpha)
    
    # Create a copy of the first tensor to modify
    mixed_tensor = tensor1.clone()
    
    # Get the bounding box for the patch
    bbx1, bby1, bbx2, bby2 = rand_bbox(tensor1.size(), lam)
    
    # Paste the patch from the second image onto the first one
    mixed_tensor[:, bby1:bby2, bbx1:bbx2] = tensor2[:, bby1:bby2, bbx1:bbx2]
    
    # Adjust lambda to reflect the actual patch size
    lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (tensor1.size()[-1] * tensor1.size()[-2]))

    # --- 4. Create and save the plot ---
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    
    img1_plot = tensor1.permute(1, 2, 0)
    img2_plot = tensor2.permute(1, 2, 0)
    mixed_img_plot = mixed_tensor.permute(1, 2, 0)

    axes[0].imshow(img1_plot)
    axes[0].set_title("Cat")
    axes[0].axis('off')

    axes[1].imshow(img2_plot)
    axes[1].set_title("Rabbit")
    axes[1].axis('off')

    axes[2].imshow(mixed_img_plot)
    axes[2].set_title(f"CutMix")
    axes[2].axis('off')

    plt.tight_layout()

    save_path = os.path.join(image_dir, "cutmix_visualization.png")
    try:
        # Use bbox_inches='tight' to prevent the title from being cut off
        plt.savefig(save_path, bbox_inches='tight')
        print(f"Successfully saved CutMix visualization to '{save_path}'")
    except Exception as e:
        print(f"Error saving the figure: {e}")

    plt.close(fig)

if __name__ == '__main__':
    TARGET_DIR = r"C:\Users\mino\Desktop\dataset\cutmix"
    CUTMIX_ALPHA = 1.0
    visualize_cutmix(TARGET_DIR, alpha=CUTMIX_ALPHA)
